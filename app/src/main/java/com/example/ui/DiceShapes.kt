...
/**
 * Official Google Android Material 3 Expressive Shapes for each dice number:
 * 1 -> Material 3 Circle (RoundedPolygon.circle)
 * 2 -> Material 3 Slanted Pill (RoundedPolygon.pill rotated at -42°)
 * 3 -> Material 3 Soft Triangle (RoundedPolygon with 3 vertices and expressive corner smoothing)
 * 4 -> Material 3 Slanted Rounded Rect (RoundedPolygon.rectangle with rounded corners and subtle slant)
 * 5 -> Material 3 Soft Pentagon (RoundedPolygon with 5 vertices and expressive corner smoothing)
 * 6 -> Material 3 Soft Hexagon (RoundedPolygon with 6 vertices and expressive corner smoothing)
 */
    ...
    fun getTransformedPolygon(number: Int): RoundedPolygon {
        return polygonCache.getOrPut(number) {
            ...
            when (number) {
                ...
                3 -> matrix.postRotate(-90f) // apex pointing up
                4 -> matrix.postRotate(-6.5f) // slanted
                5 -> matrix.postRotate(-90f) // apex pointing up
...
/**
 * Draws the official Google Material 3 Morph transition between two shapes
 */
...
/**
 * Draws a static official Material 3 Expressive shape for the given number
 */
